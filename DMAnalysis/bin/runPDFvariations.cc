...
int main(int argc, char* argv[])
{
    ...
    int evStart(0),evEnd(summaryHandler_.getEntries());
    ...
    TGraph *nomScale=new TGraph(qscale);
    nomScale->SetName("qscale_nom");
    TGraph *scaleUp=new TGraph;
    scaleUp->SetName("qscale_up");
    TGraph *scaleDown=new TGraph;
    scaleDown->SetName("qscale_down");
    for(int ip=0; ip<nomScale->GetN(); ip++) {
        Double_t x,y;
        nomScale->GetPoint(ip,x,y);
